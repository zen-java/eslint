    ...
    create(context) {
        ...
        */
        ...
        return {
            ...
            JSXElement(node) {
                const offsetFrom = tokenInfo.getFirstTokenOfLine(node);

                node.children.forEach(child => {
                    const tokens = getTokensAndComments(child);
                    offsets.setDesiredOffsets(tokens, offsetFrom, 1);
                });
            },

            JSXIdentifier(node) {
                offsets.setDesiredOffsets(getTokensAndComments(node), sourceCode.getFirstToken(node.parent), 1);
            },
            ...
            JSXOpeningElement(node) {
                ...
                node.attributes.forEach(attr => {
                    const tokens = getTokensAndComments(attr);
                    offsets.setDesiredOffsets(tokens, offsetFrom, 1);
                });

                /*
                 * Ensure the self closing portion of the tag is aligned with the first token of the line
                 * the opening tag is on:
                 *
                 * <Foo
                 *     a="b"
                 *     c="d"
                 ...
                 * ></a;
                 */
                const closingTagToken = node.selfClosing ? sourceCode.getLastToken(node, { skip: 1 }) : sourceCode.getLastToken(node);
                offsets.matchIndentOf(tokenInfo.getFirstTokenOfLine(node), closingTagToken);
            },

            JSXClosingElement(node) {
                offsets.matchIndentOf(tokenInfo.getFirstTokenOfLine(node.parent.openingElement), sourceCode.getFirstToken(node));
                offsets.matchIndentOf(tokenInfo.getFirstTokenOfLine(node), sourceCode.getLastToken(node));
            },

            JSXExpressionContainer(node) {
                offsets.setDesiredOffsets(getTokensAndComments(node.expression), sourceCode.getFirstToken(node), 1);
            },
